...
let anon_fun path = file_basename_to_find := path

let speclist =
  [
    ( "-f",
      Arg.Set_string from_path,
      "Start the search from `FROM_PATH`. Defaults to the current directory" );
  ]
...
let rec helper path tries_remaining =
  if tries_remaining <= 0 then
    failwith ("Could not find the file named: " ^ !file_basename_to_find)
  else if Sys.file_exists (Filename.concat path !file_basename_to_find) then
    path
  else helper (Filename.dirname path) (tries_remaining - 1)

(* Kickoff helper *)
let () = print_endline (helper !from_path 20)
